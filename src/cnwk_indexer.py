"""원고려기사 chunks.jsonl → Qdrant (collection: wongoryeo_gisa)."""
...
CHUNKS = ROOT / "data" / "원고려기사" / "chunks.jsonl"
...
def setup_collection(client: QdrantClient, resume: bool = False) -> int:
    ...
    for field, schema in [
        ("level1_id", qm.PayloadSchemaType.KEYWORD),
        ("level1_title", qm.PayloadSchemaType.KEYWORD),
        ("level2_id", qm.PayloadSchemaType.KEYWORD),
...
def main() -> int:
    ...
    def flush() -> None:
        if pending:
            client.upsert(COLLECTION, points=pending)
            pending.clear()
    ...
    for bs in range(start, len(chunks), ENCODE_BATCH):
        ...
        for i, c in enumerate(batch):
            ...
            pending.append(qm.PointStruct(
                id=bs + i,
                vector={
                    "dense": dvecs[i].tolist(),
                    "sparse": qm.SparseVector(
                        indices=[int(k) for k in sw.keys()],
                        values=[float(v) for v in sw.values()]),
                },
                payload={k: c[k] for k in c if k != "text_hangul_aux"} | {"text_hangul_aux": c.get("text_hangul_aux", "")},
            ))
