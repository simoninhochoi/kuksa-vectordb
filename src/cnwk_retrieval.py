...
QDRANT_PATH = ROOT / "data" / "원고려기사" / "qdrant_storage"
ENTITIES = ROOT / "data" / "원고려기사" / "entities.json"
COLLECTION = "wongoryeo_gisa"
...
def search(query: str, top_k: int = DEFAULT_TOPK, filters: dict | None = None) -> list[CnwkHit]:
    _ensure_loaded()
    expanded = expand_query(query)
    out = _state["embed"].encode([expanded], max_length=512,
    ...
    out_hits: list[CnwkHit] = []
    for sc, p in paired[:top_k]:
        pl = p.payload
        out_hits.append(CnwkHit(
            chunk_id=pl["chunk_id"], article_id=pl["article_id"], score=float(sc),
            level1_id=pl["level1_id"], level1_title=pl["level1_title"],
            level2_id=pl["level2_id"], level2_label=pl["level2_label"],
            article_title=pl["article_title"],
            date_western=pl["date_western"], date_label=pl["date_label"],
...
def get_article(chunk_id: str, with_context: bool = True) -> dict | None:
    ...
    if with_context:
        ...
        if l2:
            sres, _ = client.scroll(
                collection_name=COLLECTION,
                scroll_filter=qm.Filter(must=[qm.FieldCondition(key="level2_id", match=qm.MatchValue(value=l2))]),
                limit=100, with_payload=True,
            )
            seen: set[str] = set()
            siblings: list[dict] = []
            for pt in sorted(sres, key=lambda x: x.payload["chunk_id"]):
                xid = pt.payload["article_id"]
                if xid == aid or xid in seen:
                    continue
                seen.add(xid)
                siblings.append({"article_id": xid, "article_title": pt.payload.get("article_title", ""),
                                  "date_western": pt.payload.get("date_western", "")})
            out["year_siblings"] = siblings[:30]
    return out
...
def list_emperors() -> list[dict]:
    ...
    for _ in range(20):
        ...
        for p in res:
            l1id = p.payload["level1_id"]
            if l1id not in seen:
                seen[l1id] = {"level1_id": l1id, "level1_title": p.payload["level1_title"]}
        if not next_page:
            break
    return sorted(seen.values(), key=lambda x: x["level1_id"])


if __name__ == "__main__":
    q = " ".join(sys.argv[1:]) or "고려가 몽골에 항복하고 공물을 바치다"
    print(f"[query] {q}")
    for i, h in enumerate(search(q, top_k=5), 1):
        print(f"--- {i}. score={h.score:.4f} ---")
        print(f"  {h.level1_title} > {h.level2_label} > {h.article_title}  [{h.chunk_id}]")
        print(f"  {h.text[:200]}")
