...
QDRANT_PATH = ROOT / "data" / "고려사절요" / "qdrant_storage"
ENTITIES = ROOT / "data" / "고려사절요" / "entities.json"
COLLECTION = "goryeosa_jeolyo"
...
def search(query: str, top_k: int = DEFAULT_TOPK, filters: dict | None = None) -> list[KjHit]:
    _ensure_loaded()
    ...
    for sc, p in paired[:top_k]:
        pl = p.payload
        out_hits.append(KjHit(
            chunk_id=pl["chunk_id"], article_id=pl["article_id"], score=float(sc),
            volume_id=pl["volume_id"], volume_label=pl.get("volume_label", ""),
...
if __name__ == "__main__":
    q = " ".join(sys.argv[1:]) or "왕건이 고려를 개국하다"
