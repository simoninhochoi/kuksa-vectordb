"""고려사 엔티티 사전. data/고려사/entities.json."""
...
OUT = ROOT / "data" / "고려사" / "entities.json"
...
def build():
    ...
    for xf in tqdm(xml_files, desc="parse"):
        ...
        for title_el in tree.iter("title"):
            ...
            if main_el is None or alt_el is None: continue
    ...
    for cat, terms in table.items():
        for hanja_s in list(terms.keys()):
            if not terms[hanja_s]:
                ...
                if has_hangul(hg): terms[hanja_s].update(duum_variants(hg))
            else:
                more = set()
                for hg in list(terms[hanja_s]): more |= duum_variants(hg)
                terms[hanja_s] |= more
...
def main():
    ...
    for cat in sorted(table.keys()):
        print(f"  {cat:10s} {len(table[cat]):>6d}")
    return 0
