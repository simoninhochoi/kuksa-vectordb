...
QDRANT_PATH = ROOT / "data" / "고려사" / "qdrant_storage"
ENTITIES = ROOT / "data" / "고려사" / "entities.json"
COLLECTION = "goryeosa"
...
class KrHit:
    ...
    score: float
    volume_id: str
    section_type: str          # 世家/志/列傳/序文/고려세계 등
    volume_label: str
...
def search(query: str, top_k: int = DEFAULT_TOPK, filters: dict | None = None) -> list[KrHit]:
    _ensure_loaded()
    ...
    for sc, p in paired[:top_k]:
        pl = p.payload
        out_hits.append(KrHit(
            chunk_id=pl["chunk_id"], article_id=pl["article_id"], score=float(sc),
            volume_id=pl["volume_id"], section_type=pl.get("section_type", ""),
            volume_label=pl.get("volume_label", ""),
            breadcrumbs=pl.get("breadcrumbs", []),
            article_title=pl["article_title"],
            date_western=pl["date_western"], date_label=pl.get("date_label", ""),
            ganji=pl.get("ganji", ""),
...
if __name__ == "__main__":
    q = " ".join(sys.argv[1:]) or "태조가 후삼국을 통일하다"
    for i, h in enumerate(search(q, top_k=5), 1):
        print(f"--- {i}. score={h.score:.4f} ---")
        bc = " > ".join(h.breadcrumbs)
        print(f"  {bc} > {h.article_title}  [{h.chunk_id}]")
        print(f"  {h.text[:200]}")
