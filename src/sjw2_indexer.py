"""승정원일기 chunks.jsonl 후반부 → Qdrant (collection: sjw2).

순조~순종 (chunk index 1,258,764~) 인덱싱 전용. 별도 storage path 사용.
sjw collection은 인조~정조까지 1.26M points 보유 (정조 끝 idx=1,258,763).
순조 partial (idx 1,258,764~1,262,594) 4k는 sjw collection에 잔존하나
retrieval 단계에서 king_prefix 필터로 제외.
"""
...
QDRANT_PATH = ROOT / "data" / "승정원일기" / "qdrant_storage_sjw2"
COLLECTION = "sjw2"
...
# 순조 (H) 시작 chunk index. 인조~정조는 sjw collection에서 처리됨.
START_CHUNK_INDEX = 1_258_764
